...
export default function Home() {
  ...
  // 1️⃣ Single-shot getCurrentLoc on user action
  const getCurrentLoc = () => {
    setError("");
    if (!navigator.geolocation) {
      setError("Geolocation is not supported by your browser.");
  ...
  // 2️⃣ Whenever location changes (after permission), fetch nearby requests
  useEffect(() => {
    if (!location) return;
    const fetchNearby = async () => {
      try {
  ...
  // 3️⃣ Socket setup for real‑time acceptance
  useEffect(() => {
    if (!token || !user?._id) return;
    const socket = io(
  ...
  const handleHelpSubmit = async (e) => {
    ...
    if (!location) {
      setError("Please enable location before posting a request.");
      return;
    }
    try {
      await api.post(
    ...
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      <header className="flex justify-between items-center">
        ...
        <div className="space-x-2 flex items-center">
          ...
          {/* 4️⃣ User‑initiated location permission */}
          <button
            onClick={getCurrentLoc}
            className="ml-2 px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600"
          >
            {location ? "Refresh Location" : "Enable Location"}
          </button>
        </div>
      </header>
      ...
      <section>
        <h2 className="text-lg font-semibold text-orange-600 mb-2">
          Nearby Help Requests (10 km)
        </h2>
        {nearbyRequests.length === 0 ? (
          <p className="text-gray-500">
            {location
              ? "No requests found near you."
