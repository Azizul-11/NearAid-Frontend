...
export default function ChatPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-100">
      <header className="flex flex-col sm:flex-row items-center justify-between bg-white px-4 sm:px-6 py-3 shadow-md">
        <button onClick={() => navigate('/home')} className="text-orange-600 hover:underline mb-2 sm:mb-0">
          ← Home
        </button>
      ...
      <div ref={chatBoxRef} className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-4">
        {messages.length === 0 ? (
          <p className="text-center text-gray-500">No messages yet</p>
        ) : (
          messages.map((msg, i) => {
            ...
            return (
              <div key={i} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}> 
                <div className={`max-w-full sm:max-w-xs px-4 py-2 rounded-lg space-y-1 ${isMine ? 'bg-orange-600 text-white' : 'bg-white text-gray-800 shadow-md'}`}>
                  {msg.type === 'location' ? (
                    <>
                      <a
                        href={`https://www.google.com/maps?q=${msg.latitude},${msg.longitude}`}
      ...
      <div className="bg-white px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center border-t space-y-2 sm:space-y-0 sm:space-x-2">
        <button
          onClick={shareLocation}
          disabled={!socketReady || sharing}
          className={`w-full sm:w-auto px-4 py-2 rounded ${sharing ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 text-white hover:bg-blue-600'}`}
        >
          {sharing ? 'Sharing...' : 'Share Location'}
        </button>
